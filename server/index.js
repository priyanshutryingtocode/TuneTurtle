...
function validateSongRequest(artist, song) {
    ...
    if (!cleanArtist || !cleanSong) return { error: "Artist and song title are required." };
...
app.post("/api/analyze", async (req, res) => {
    ...
    const searchKey = `${artist.toLowerCase()}-${song.toLowerCase()}`.replace(/\s+/g, '-');

    try {
        if (memoryCache.has(searchKey)) {
            console.log(`\nCache HIT: Serving "${song}" from Server RAM`);
            const cachedData = memoryCache.get(searchKey);
            return res.json({
                success: true,
        ...
        console.log(`\nCache MISS: Generating analysis for "${song}" by ${artist}`);

        const searches = await withTimeout(Client.songs.search(`${song} ${artist}`), "Genius search");
        ...
        try {
            ...
            console.log(`Genius failed. Engaging LRCLIB fallback...`);
            try {
                const cleanTitle = firstSong.title.replace(/\s*\(.*?\)/g, '').replace(/\s*-.*$/, '').trim();
                const query = encodeURIComponent(`${firstSong.artist.name} ${cleanTitle}`);
                
                const fallbackResults = await fetchJsonWithTimeout(`https://lrclib.net/api/search?q=${query}`, "LRCLIB Search");
                if (!fallbackResults || fallbackResults.length === 0) throw new Error("No results");
                
                const validTrack = fallbackResults.find(track => 
                    (track.plainLyrics && track.plainLyrics.length > 50) || 
                    (track.syncedLyrics && track.syncedLyrics.length > 50)
                );
                
                if (!validTrack) throw new Error("No lyrics data found");
                
                if (validTrack.plainLyrics) {
                    lyrics = validTrack.plainLyrics;
                    console.log("Lyrics fetched from LRCLIB (Plain text)");
                } else {
                    lyrics = validTrack.syncedLyrics.replace(/\[\d{2}:\d{2}\.\d{2,3}\]/g, '').trim();
                    lyrics = lyrics.replace(/^\s*[\r\n]/gm, '');
                    console.log("Lyrics fetched from LRCLIB (Stripped synced text)");
                }
            } catch (fallbackErr) {
                console.error("Fallback failed:", fallbackErr.message);
                return res.status(500).json({ success: false, error: "Could not retrieve lyrics. Both sources blocked." });
            }
        }
        ...
        if (!lyrics) return res.status(404).json({ success: false, error: "Lyrics found but came back empty." });
        ...
        const aiData = normalizeAnalysis(JSON.parse(extractJsonObject(result.response.text())));
        ...
        const trackData = { 
            ...
            producers, writers, album               
        };

        memoryCache.set(searchKey, {
            track: trackData,
            lyrics: lyrics,
            analysis: aiData
        });
        ...
        console.log(`Saved "${song}" to RAM Cache`);
        ...
        if (err.message.includes("503")) return res.status(503).json({ success: false, error: "Google AI is overloaded." });
...
app.post("/api/chat", async (req, res) => {
    ...
    if (!message || !lyrics) return res.status(400).json({ success: false, error: "Missing required chat data." });
